...
function Avatar({
  ...
  return (
    <AvatarPrimitive.Root
      ...
      data-size={size}
      className={cn(
        "group/avatar relative flex size-8 shrink-0 items-center rounded-full select-none *:absolute after:absolute after:inset-0 after:rounded-full after:border after:border-border after:mix-blend-darken data-[size=lg]:size-10 data-[size=sm]:size-6 dark:after:mix-blend-lighten",
        className,
      )}
      {...props}
    />
...
> &
  Partial<
    Pick<
      ...
      "src" | "layout" | "width" | "height"
    >
  >

function UnpicAvatar({ src = "", layout, ...props }: UnpicAvatarProps) {
  const imageProps = { src, layout, ...props } as React.ComponentProps<
    typeof Image
  >
